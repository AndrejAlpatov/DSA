""" This module contains the handler for the OpeningTimeIntent """
...
class OpeningTimesIntentHandler(AbstractRequestHandler):
    ...
    def can_handle(self, handler_input):
        ...
        return is_intent_name("OpeningTimeIntent")(handler_input)
    ...
    def handle(self, handler_input):
        ...
        mensa_department = slots['MensaDepartment'].value

        # If slot value is kiosk
        if mensa_department in slot_values_for_kiosk:
            # Zufällige Antwortauswahl aus der Liste
            list_index = randint(0, len(speech_text_kiosk_open_times_answers) - 1)

            speech_text = speech_text_kiosk_open_times_answers[list_index]

        # If slot value is mensa
        elif mensa_department in slot_values_for_mensa:
            # Zufällige Antwortauswahl aus der Liste
            list_index = randint(0, len(speech_text_mensa_open_times_answers) - 1)

            speech_text = speech_text_mensa_open_times_answers[list_index]
